...
Elm.Accelerate = Elm.Accelerate || {};
Elm.Accelerate.make = function (_elm) {
   "use strict";
   _elm.Accelerate = _elm.Accelerate || {};
   if (_elm.Accelerate.values) return _elm.Accelerate.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var view = F2(function (_p0,model) {
      ...
      var _p3 = _p1._0;
      var _p2 = _p1._1;
      var c = A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: model.currentPoint.x * $Basics.toFloat(_p3),_1: model.currentPoint.y * $Basics.toFloat(_p2)},
      A2($Graphics$Collage.filled,$Color.red,$Graphics$Collage.circle(model.currentPoint.z * $Basics.toFloat(_p3))));
      return A3($Graphics$Collage.collage,_p3,_p2,_U.list([c]));
   });
   var pointToNote = function (point) {    return $Basics.round(200 + $Basics.abs((point.x + point.y + point.z) * 100));};
   var update = F2(function (action,model) {
      ...
      if (_p4.ctor === "NoOp") {
            ...
            var _p5 = _p4._0;
            return _U.update(model,{currentPoint: _p5,currentNote: pointToNote(_p5)});
         }
   });
   var AddPoint = function (a) {    return {ctor: "AddPoint",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var inbox = $Signal.mailbox(NoOp);
   var initialModel = {currentNote: 0,currentPoint: {x: 0,y: 0,z: 0}};
   var Model = F2(function (a,b) {    return {currentNote: a,currentPoint: b};});
   var acceleration = Elm.Native.Port.make(_elm).inboundSignal("acceleration",
   "Accelerate.Point",
   function (v) {
      return typeof v === "object" && "x" in v && "y" in v && "z" in v ? {_: {}
                                                                         ,x: typeof v.x === "number" ? v.x : _U.badPort("a number",v.x)
                                                                         ,y: typeof v.y === "number" ? v.y : _U.badPort("a number",v.y)
                                                                         ,z: typeof v.z === "number" ? v.z : _U.badPort("a number",
                                                                         v.z)} : _U.badPort("an object with fields `x`, `y`, `z`",v);
   });
   var sampledAcceleration = A2($Signal.sampleOn,$Time.fps(30),acceleration);
   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,AddPoint,sampledAcceleration));
   var state = A3($Signal.foldp,update,initialModel,actions);
   var note = Elm.Native.Port.make(_elm).outboundSignal("note",function (v) {    return v;},A2($Signal.map,function (_) {    return _.currentNote;},state));
   var main = A3($Signal.map2,view,$Window.dimensions,state);
   var Point = F3(function (a,b,c) {    return {x: a,y: b,z: c};});
   return _elm.Accelerate.values = {_op: _op
                                   ,Point: Point
                                   ,Model: Model
                                   ...
                                   ,actions: actions
                                   ,state: state
                                   ,sampledAcceleration: sampledAcceleration
                                   ,main: main};
};
